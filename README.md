...
- Right-click VM2 and select `Settings`:
  - Click `Add Device` and select `Network Adapter`;
  - Select `vmvnet2`;
  - Select `Show All` and select `Network Adapter` once again and add a second adapter, this time selecting `vmvnet3`;
...
To make the `iptables` rules persistent, in VM2 install `iptables-persistent` (select "yes" to save current rules):
